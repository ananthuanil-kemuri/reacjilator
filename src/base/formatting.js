...
const formatTextForMentions = text => {
  ...
  const formatForUserMentions = text =>
    text.replace(/<\s*@\s*/g, stripWhitespace)
  // Links to public channels should look like "<#C024BE7LR>" or "<#C024BE7LR|some-channel>"
  const formatForChannelMentions = text =>
    text.replace(/<\s*#\s*[A-Z0-9]*\s*\|\s*[a-zA-Z0-9-]*\s*>/g, stripWhitespace)
  ...
  return formatForSpecialMentions(
    formatForChannelMentions(formatForUserMentions(text))
  )
}
